...
const fetchaMovie = async id => {
  const respone = await fetch(
    "https://api.themoviedb.org/3/movie/" +
      id +
      "?language=en-US&api_key=4e44d9029b1270a757cddc766a1bcb63"
  );
...
const fetchGenre = async () => {
  const respone = await fetch(
    "https://api.themoviedb.org/3/genre/movie/list?language=en-US&api_key=4e44d9029b1270a757cddc766a1bcb63"
  );
  const data = respone.json();
  if (respone.status > 400) {
...
const fetchRecMovies = async id => {
  const respone = await fetch(
    "https://api.themoviedb.org/3/movie/" +
      id +
      "/recommendations?page=1&language=en-US&api_key=4e44d9029b1270a757cddc766a1bcb63"
  );
  const data = respone.json();
  if (respone.status > 400) {
    throw new Error(data.error);
